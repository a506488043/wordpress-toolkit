...
(function($) {
    ...
    $(document).ready(function() {
        // 安全检查：确保 SaiitaAIToolkitTagOptAdmin 已定义
        if (typeof SaiitaAIToolkitTagOptAdmin === 'undefined') {
            console.error('SaiitaAIToolkitTagOptAdmin 未定义，JS变量注入失败');
            return;
        }

        // 统计信息
        var stats = SaiitaAIToolkitTagOptAdmin.stats || {};

        // 检查免费次数，置灰按钮
        function checkUsageLimits() {
            var isPro = SaiitaAIToolkitTagOptAdmin.isPro;
            var remaining = parseInt(SaiitaAIToolkitTagOptAdmin.remaining, 10);

            // 批量按钮：次数用完置灰
            if (!isPro && remaining <= 0) {
                $('#batch-generate-descriptions').prop('disabled', true).addClass('button-secondary').removeClass('button-primary').attr('title', '免费次数已用完，请升级PRO');
        ...
        // 适配字段名映射
        if (stats.tags_without_description !== undefined) {
            stats.without_description = stats.tags_without_description;
        }
        ...
        function updateProgress(title, percentage, processed, success, errors, currentCategory, totalCount) {
            ...
            $('#current-tag').text(currentCategory);
        ...
        $('#batch-generate-descriptions').on('click', function(e) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    nonce: SaiitaAIToolkitTagOptAdmin.batchNonce
                },
                beforeSend: function() {
        ...
        $('.generate-description-single').on('click', function(e) {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    tag_id: tagId,
                    nonce: SaiitaAIToolkitTagOptAdmin.nonce
                },
                success: function(response) {
                    if (response.success) {
                        var data = response.data;
