 ...
 */
class Saiita_AI_Toolkit_Category_Optimization
{
    ...
    private function load_settings()
    {
        ...
        $saved_settings = get_option('saiita_ai_content_toolkit_category_optimization_settings', array());
    ...
    public function activate()
    {
        ...
        try {
            // 创建默认设置（仅在不存在时）
            if (!get_option('saiita_ai_content_toolkit_category_optimization_settings')) {
                add_option('saiita_ai_content_toolkit_category_optimization_settings', $this->settings);
            } else {
            }
        } catch (Exception $e) {
        }
    ...
    {
        // 只在分类优化页面加载
        if (strpos($hook, 'saiita-ai-content-toolkit-category-optimization') === false) {
            return;
        }
            // 加载共享管理后台样式
            wp_enqueue_style(
            ...
            // 加载模块特定样式（如果有）
            wp_enqueue_style(
                'saiita-ai-content-toolkit-category-optimization-admin',
                SAIITA_AI_CONTENT_TOOLKIT_PLUGIN_URL . 'modules/category-optimization/assets/css/admin.css',
            ...
            // 加载共享管理功能脚本
            wp_enqueue_script(
                'saiita-ai-content-toolkit-shared-admin-functions',
                SAIITA_AI_CONTENT_TOOLKIT_PLUGIN_URL . 'assets/js/shared-admin-functions.js',
                array('jquery'),
                SAIITA_AI_CONTENT_TOOLKIT_VERSION,
                true
            );
            ...
            wp_enqueue_script(
                'saiita-ai-content-toolkit-category-optimization-admin',
                SAIITA_AI_CONTENT_TOOLKIT_PLUGIN_URL . 'modules/category-optimization/assets/js/admin.js',
                array('jquery', 'saiita-ai-content-toolkit-shared-admin-functions'),
                SAIITA_AI_CONTENT_TOOLKIT_VERSION,
                true
            );
            ...
            // 免费版本无使用限制
            wp_localize_script('saiita-ai-content-toolkit-category-optimization-admin', 'SaiitaAIToolkitCategoryOptAdmin', array(
                'nonce' => wp_create_nonce('saiita_category_optimization_nonce'),
                'batchNonce' => wp_create_nonce('saiita_category_optimization_batch'),
                'stats' => $stats,
    ...
    public function update_settings($new_settings)
    {
        ...
        update_option('saiita_ai_content_toolkit_category_optimization_settings', $this->settings);
    }
    ...
    public function generate_category_description($category_id)
    {
        ...
        try {
            ...
            if (!$category) {
                return array('success' => false, 'message' => __('分类不存在', 'saiita-ai-content-toolkit'));
            }
            ...
            if (empty($posts)) {
                $articles_content = '该分类下暂无文章，请根据分类名称推断其含义。';
                $keywords_text = $category->name;
            } else {
                // 分析文章内容
                $articles_content = '';
                $keywords = array();
            ...
            if (function_exists('saiita_ai_content_toolkit_get_prompt')) {
                $prompt = saiita_ai_content_toolkit_get_prompt('category_optimization', array(
...
4. 只返回描述内容，不要包含其他解释";
            ...
            if (is_wp_error($response)) {
                if (defined('WP_DEBUG_LOG') && WP_DEBUG_LOG) {
                    error_log('AI API Error (category): ' . $response->get_error_code() . ' - ' . $response->get_error_message()); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
                }
            }
            ...
            return array('success' => false, 'message' => __('AI生成分类描述失败：', 'saiita-ai-content-toolkit') . $e->getMessage());
        }
    }
    ...
    public function apply_category_description($category_id, $description)
    {
        ...
        try {
            ...
            return array(
                'success' => true,
                'message' => __('分类描述更新成功', 'saiita-ai-content-toolkit'),
                'category_id' => $category_id,
                'category_name' => $category->name
            );
        } catch (Exception $e) {
            return array('success' => false, 'message' => __('分类描述更新失败：', 'saiita-ai-content-toolkit') . $e->getMessage());
        }
    }
    ...
    public function batch_generate_descriptions()
    {
        ...
        try {
            ...
            if (is_wp_error($categories)) {
                return array(
                    'success' => false,
                    'message' => __('获取分类列表失败：', 'saiita-ai-content-toolkit') . $categories->get_error_message()
                );
            }

            // 批量描述生成不过滤，0篇文章的分类也需要描述
            $valid_categories = $categories;

            if (empty($valid_categories)) {
                return array(
                    'success' => true,
                    'message' => __('没有符合条件的分类需要处理', 'saiita-ai-content-toolkit'),
                    'processed_count' => 0,
                    'success_count' => 0,
                    'error_count' => 0
                );
            ...
            foreach ($valid_categories as $category) {
                ...
                try {
                    // 检查使用次数限制
                    $license_manager = Saiita_AI_Toolkit_License_Manager::get_instance();
                    $can_use = $license_manager->can_use('category_optimization');
                    if (!$can_use['allowed']) {
                        break; // 达到上限，停止批量处理
                    }
                    ...
                    if ($result && $result['success']) {
                        ...
                        if ($apply_result && $apply_result['success']) {
                            $success_count++;
                            // 增加使用次数
                            $license_manager->increment_usage('category_optimization');
                        } else {
                            $error_count++;
                        }
                    } else {
            ...
            return array(
                ...
                'message' => sprintf(
                    ...
                    esc_html__('批量生成分类描述完成！处理：%1$d个，成功：%2$d个，失败：%3$d个', 'saiita-ai-content-toolkit'),
                    $processed_count,
                    $success_count,
                    $error_count
                )
            ...
            return array(
                'success' => false,
                'message' => __('批量生成分类描述失败：', 'saiita-ai-content-toolkit') . $e->getMessage()
            );
        }
    }
    ...
    {
        // 验证nonce
        if (!wp_verify_nonce(sanitize_text_field(wp_unslash(isset($_POST['nonce']) ? sanitize_key(wp_unslash($_POST['nonce'])) : '')), 'saiita_category_optimization_nonce')) { // phpcs:ignore WordPress.Security.NonceVerification, WordPress.Security.ValidatedSanitizedInput
    ...
    {
        // 验证nonce
        if (!wp_verify_nonce(sanitize_text_field(wp_unslash(isset($_POST['nonce']) ? sanitize_key(wp_unslash($_POST['nonce'])) : '')), 'saiita_category_optimization_batch')) { // phpcs:ignore WordPress.Security.NonceVerification, WordPress.Security.ValidatedSanitizedInput
            wp_send_json_error(array('message' => '安全验证失败'));
            return;
        }
    ...
    public function ajax_get_categories_list()
    {
        ...
        try {
            ...
            wp_send_json_error(array('message' => __('获取分类列表失败：', 'saiita-ai-content-toolkit') . $e->getMessage()));
        }
    }
    ...
    public function admin_page()
    {
        ...
        $admin_page = Saiita_AI_Toolkit_Category_Optimization_Admin::get_instance();
        $admin_page->admin_page();
    }
}

// 注册插件激活和停用钩子
register_activation_hook(__FILE__, array('Saiita_AI_Toolkit_Category_Optimization', 'activate'));
register_deactivation_hook(__FILE__, array('Saiita_AI_Toolkit_Category_Optimization', 'deactivate'));
